...
const Page = ({ totalPage, page, nextPage, prevPage, firstPage, lastPage }) => {
  ...
  const generatedPage = () => {
    switch (currentItem) {
      case (totalPage - 1): 
        return [currentItem - 1, currentItem, currentItem + 1];
      case totalPage:
        return [currentItem - 2, currentItem - 1, currentItem];
      default: return [currentItem, currentItem + 1, currentItem + 2]
    }
  }
  ...
  const changeLastPage = () => {
    lastPage()
    setCurrentItem(totalPage)
  }

  return (
    <div className="page">
      <div className="page__wrapper">
        <button className="btn btn-primary" disabled={cn({disabled:currentItem===1})}  onClick={changeFirstPage}><svg width="14" height="13" viewBox="0 0 14 13" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <button className="btn btn-primary" disabled={cn({disabled:currentItem===1})} onClick={changePrevPage}><svg width="9" height="13" viewBox="0 0 9 13" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="page__wrapper_pagination" id="wrapperPageNumbers">
            {pageNumbers.map(number => (
              <div ref={link} className={cn('page__pagination-item', {'page__pagination-item_active':number===currentItem})} key={number}>
                <a href="!#" className="page__pagination-link" onClick={() => {page(number); setCurrentItem(number)}}>
                  {number}
                </a>
              </div>
            ))}
          </div>
        <button className="btn btn-primary" disabled={cn({disabled:currentItem === (pageNumbers[pageNumbers.length - 1])})} onClick={changeNextPage}><svg width="9" height="13" viewBox="0 0 9 13" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <button className="btn btn-primary" disabled={cn({disabled:currentItem === (pageNumbers[pageNumbers.length - 1])})} onClick={changeLastPage}><svg width="14" height="13" viewBox="0 0 14 13" fill="none" xmlns="http://www.w3.org/2000/svg">
